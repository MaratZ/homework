...
@pytest.fixture
def filter_fixture():# type: ignore
...
@pytest.fixture
def filter_fixture_no_state():# type: ignore
...
@pytest.fixture
def sort_the_same_dates():# type: ignore
    return [{'id': 41428829, 'state': 'EXECUTED', 'date': '2019-07-03T18:35:29.512364'},
...
@pytest.mark.parametrize('state, expected', [
        ...
        ('CANCELED', [{'id': 594226727, 'state': 'CANCELED', 'date': '2018-09-12T21:27:25.241689'},
         {'id': 615064591, 'state': 'CANCELED', 'date': '2018-10-14T08:21:33.419441'}])])
def test_filter_by_state(filter_fixture, state, expected):# type: ignore
...
def test_sort_by_date(filter_fixture, keys, expected):# type: ignore
    assert sort_by_date(filter_fixture, keys) == expected


def test_filter_by_state_invalid_state(filter_fixture_no_state):# type: ignore
    with pytest.raises(ValueError):
        filter_by_state(filter_fixture_no_state)


def test_sort_by_date_the_same_dates(sort_the_same_dates):# type: ignore
