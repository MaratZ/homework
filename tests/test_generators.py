...
def test_transaction_descriptions_3(index, expected):# type: ignore
    descriptions = list(transaction_descriptions(transactions))
    assert descriptions[index] == expected


def test_transaction_descriptions_zero():# type: ignore
    with pytest.raises(SystemExit, match="Нет транзакций") as exc_info:
...
def test_card_number_generator_beginning():# type: ignore
    numer = card_number_generator(0, 1)
    assert next(numer) == '0000 0000 0000 0000'


def test_card_number_generator_end():# type: ignore
