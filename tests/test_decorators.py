...
def test_log_print(capsys):# type: ignore
    my_function(10, 5)
...
def test_log_print_try(capsys):# type: ignore
    my_function(10, 0)
    captured = capsys.readouterr()
    expected_output = (
        'my_function started\n'
...
def test_log_print_fail(tmp_path):# type: ignore
...
def test_log_print_fail_try(tmp_path):# type: ignore
    log_file = tmp_path / "test_output.txt"
